...
<template>
  <BwWorkspace :progress-pct="86" progress-note="18 von 21 Entscheidungen · ~10 Min" progress-to="/brand/demo/ergebnis" :score="87" content-locale="de">
    <template #brand>
      <BwBrandSwitcher :current="{ title: 'Schubert UX Studio', path: 'Marken-Relaunch', flag: 'i-circle-flags-de' }" :others="[{ title: 'Kailua Coffee Co.', path: 'Neue Marke', flag: 'i-circle-flags-us', to: '/brand/demo/werte' }]" />
    </template>
    ...
    <template #default>
      ...
      <h1 class="mt-1 mb-6 text-4xl leading-tight">Welcher Charakter ist eure Marke?</h1>
      <BwPairCompare :a="pairs[Math.min(round, pairs.length - 1)]!.a" :b="pairs[Math.min(round, pairs.length - 1)]!.b" @pick="pick" />
      <p class="bw-label mt-4" style="color: var(--bw-muted)">Paar {{ Math.min(round + 1, 8) }} von 8 · Bauchgefühl reicht</p>
    </template>
