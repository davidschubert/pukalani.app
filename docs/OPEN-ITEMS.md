...
**Stand: 8 offen · 2 geparkt/wartend · 13 bewusst zurückgestellt** (Zahlen bei JEDEM Umzug nach COMPLETE mitführen)
...
| PM1 | 8 — Stunden | **Produkt-Manifest-Wächter.** Ein Manifest mit `apiPrefixes` wirkt erst, wenn ein `server/plugins/product-manifest.ts` es zur Laufzeit registriert — `market` fuhr seit M1 ohne (Notabschaltung tot, Rohtext-Sweep lief nie; behoben 2026-09-09, COMPLETE „BI1 I1"). Drei Layer haben weiter keins: `domains`, `onboarding`, `runner` — je entscheiden, ob gewollt; dann Regel in `scripts/check-manifests.mjs`: `apiPrefixes` ⇒ Plugin Pflicht (mit Ausnahmeliste). | Mittel | S | Ja: Ausnahmen (domains/onboarding/runner) | — |
