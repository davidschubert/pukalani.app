...
| 9 · MV1 | **Marktvergleich — M1, M2 und M3 GEBAUT.** M3: Bericht `market-r-1` (Matrix + drei Listen), § 6 UWG-Riegel mit Gegenproben, Markt-Befunde in `brand_findings` (sperren nichts), Idempotenz + `stale`, Bibliotheks-Mechanik, Brand-Check-Score gelesen, George-Block; Vertrag Anhang D, `verify-market-report.mjs` 43/43. Als Nächstes **M4** (Oberfläche), danach M5+M6. **Prod-Migrationen `market` 001–003 UND `brand-018` auf `branding` stehen aus** (brand-018 muss VOR dem Deploy laufen, sonst verschwinden Markt-Befunde still). | Mittel | L | Ja: Schema-Anhang B + Prod-Migrationen freigeben | [BRAND-MARKTVERGLEICH.md](plans/BRAND-MARKTVERGLEICH.md) |
