...
| --- | --- | --- | --- | --- | --- |
| 1 · A0 | **Naht-Geheimnisse — der Rest der Zugänge.** KI, Statistik, Konsolen-KI und seit heute **SMTP** lassen sich unter Instanz → Integrationen eintragen (verschlüsselt, `instance_secrets`). Offen bleiben nur die GETEILTEN Geheimnisse zwischen platform und admin (`onboardingServiceSecret`, `eventsSweepKey`): sie müssen auf beiden Seiten gleich sein, ein Umzug braucht eine Stufe, in der beide Seiten alten UND neuen Wert annehmen. Nicht dringend — über die Env läuft alles weiter. | Niedrig | M | Nein | [instanceSecrets.ts](../packages/core/server/utils/instanceSecrets.ts) |
