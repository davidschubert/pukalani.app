...
| 10 · DB1 | **Discover Brands — öffentliche Markengalerie auf branding.supply.** Der Klickdummy aus Runde 97 liegt im brand-Playground (`/brand/demo/discover`: Kacheln in der Farbwelt der Marke, Filter Alle/Neue Marke/Relaunch, Archetyp-Facetten, Sortierung Brand Score/Bewertung/Trending/Neueste). Die Daten gibt es seit BC1/MV1 (Brand-Check-Score mit Ranking-Opt-in, Archetyp und Kategorie aus dem Wizard, Freigabe fremder Marken). Weg nach WORKFLOW.md: Strategie + Konzept (Opt-in-Regel, was öffentlich ist, Rechtsrahmen wie Marktvergleich Anhang G) → Prototyp am Dummy → Freigabe → Pakete. Nav-Punkt „Discover Brands" kehrt erst mit der echten Seite zurück (404-Audit 2026-09-03). Reihenfolge: nach BF1 und dem Strang „Brand System" (Produkt 02/03), sofern David nichts vorzieht. | Mittel | M | Ja: Konzept-Entscheidungen, Prototyp-Abnahme | Dummy: packages/brand/.playground · Vormerkung: [BRAND-WIZARD-PHASE-1.md](plans/BRAND-WIZARD-PHASE-1.md) „Journal & Content-Intelligence" |
...
| BI1 | 5 — Wochen | **Brand Insights — redaktioneller Bereich (früher „Journal", umbenannt 2026-08-30).** Klickdummy aus Runde 103 im brand-Playground (`/brand/demo/journal`, `/artikel`, `/profil`, `/duell`, `/anatomie`: Kategorien, Sortierung, Grid/Liste per `?display=`). Inhalt: Entity-Seiten externer Marken, Rankings, Themencluster, Artikel, Duelle; Content-Pipeline mit YouTube als Research-Signal (Vormerkung Phase-1-Plan). Braucht ZUERST ein Redaktionskonzept (wer schreibt, wie oft, Rechtsrahmen für Bewertungen fremder Marken — Marktvergleich Anhang G deckt einen Teil) und baut auf Discover Brands (DB1) auf. | Mittel | L | Ja: Redaktionskonzept | DB1 + Redaktionskonzept |
