...
| 8 · BC1 | **Brand-Check als Produktseite — P1–P6 GEBAUT + LIVE 2026-09-06 (Details COMPLETE), wartet auf Davids Abnahme.** `/brand-check` mit Reitern Start/Ranking/Vergleich, Markenabdruck, Ergebnisseite v2 (Fazit-Bento, Ampel-Matrix, Verbesserungsplan mit Punktgewinn), Korrektur-Warteschlange, Meine Brands/Brand-Scores, Dokument-Check. Migration brand-017 gefahren. OFFEN: Davids Test der Ergebnisseite; nicht live durchgespielt (Login, KI-Kosten): Dokument-Check, Neu-ermitteln, Korrektur-Annahme; Folge: leere `UTable`-Spaltenköpfe (`header: () => ''`) in ~40 Stellen anderer Layer erzeugen stille Hydration-Fehler. | Hoch | S (Rest) | Ja: Abnahme, Dokument-Check einmal klicken | [BRAND-CHECK-SEITE.md](plans/BRAND-CHECK-SEITE.md) |
