...
| U21 | 2 — Tage | **Tickets, die sich selbst umsetzen.** Ein Knopf am Ticket startet Claude Code auf Davids Mac — eigener Worktree, Beschreibung als Auftrag, Anhänge als Material, Modell und Berechtigungs-Modus aus dem Ticket; Fortschritt live im Board, danach Branch, Commit, Diffstat, Tests und Kosten im Ticket. Das Board steht schon, es fehlt die Ausführungs-Schicht: neuer Layer `agents` plus ein kleiner Dienst auf dem Mac (die Website kann selbst nichts starten). Entscheidungen gefallen (2026-08-17): eigener Layer · Polling statt Realtime · headless zuerst. **Zwei Riegel sind Pflicht:** die Liste erlaubter Repos liegt auf dem Mac, nicht in der Datenbank, und Gast-Feedback darf nie zur Anweisung werden. | Mittel | M | Nein (Entscheidungen sind gefallen) | [AI-RUNNER.md](plans/AI-RUNNER.md) |
