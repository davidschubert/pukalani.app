...
| BI2 | 6 — Tage | **Brand Insights: Kuration, Merkliste, Artikel aus Video.** Davids sieben Wünsche vom 2026-09-09 zum Themenradar, als Konzept geschrieben: Kanalliste einsehen/bearbeiten/einschränken (heute nur im Code, `apps/branding/app/app.config.ts`), Cluster erweitern (`seo`/`geo` trennen, `brand-ai` neu, „Brand Design"), Filtern + Sortieren statt fest nach Opportunity, eigene Fundstück-Liste per YouTube-Link, Dreifach-Schalter „Nicht geprüft/Uninteressant/Interessant" in BEIDEN Listen (das Urteil lebt in der Merkliste, nicht auf der flüchtigen Radar-Zeile), Filter „nur Interessante", KI-Brief aus einem Video + SEO/GEO-Artikelregeln, Kommentare unter Artikeln (erst nach I3). **Sieben Pakete K1–K7**; K1+K2 ohne Gate und ohne Migration, K3+K4 je eine Prod-Migration (`insights-005`/`006`), K7 nach I3. | Mittel | M–L | **Ja: drei Entscheidungen (A Kanalliste in Tabelle? · B Cluster-Katalog erweitern? · C wie tief liest die KI das Video?)** | [BRAND-INSIGHTS-KURATION.md](plans/BRAND-INSIGHTS-KURATION.md) |
