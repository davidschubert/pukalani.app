...
| 9 · MV1 | **Marktvergleich — gebaut und live; offen sind nur noch Davids Punkte.** (1) **Rechts-Check der Namensnennung** mit dem Anwalt — die sechs Fragen stehen im Archiv, Anhang G. (2) **Handprüfung der Entwürfe `meta` (Nike und Anthropic übernommen 2026-09-06, lib-5; Meta liegen gelassen)** (liegen lokal unter `drafts/`, gitignored). (3) **Auffrischung nach 90 Tagen** steht heute nur im Runbook, nicht im Code. (4) BC1-Nachzug: `runBrandCheck()` exportieren, geteilten Mehrseiten-Abruf übernehmen. | Mittel | S–M | Ja: Rechts-Check, Handprüfung | [archiv/BRAND-MARKTVERGLEICH.md](archiv/BRAND-MARKTVERGLEICH.md) · [Einführung](runbooks/MARKTVERGLEICH-EINFUEHRUNG.md) · [Bibliothek](runbooks/MARKTVERGLEICH-BIBLIOTHEK.md) |
| 10 · BF1 | **Brand Foundation als Guidelines — Konzept FREIGEGEBEN 2026-09-05 (vier Entscheidungen nach Empfehlung); PROTOTYP GEBAUT 2026-09-05 (Playground 3009: `/brand/demo/foundation`, `?richtung=warm`, Teilen-Dialog, Export-Menü, `/brand/demo/share`, Druck) — Davids Abnahme offen.** Ein Dokument, zwei Ansichten: „Euer Branding" bleibt Arbeitsansicht, die **Brand Foundation** (`/brand/:id/foundation`) liest dieselben bestätigten Werte als Markenhandbuch (Story, Kontext, Purpose/Vision/Mission, Positionierung, Architektur, Werte, Stimme mit Do & Don't, Manifest, Tagline & Messaging, Name, visuelle Schranke, KI-Regeln) — nur Festlegungen (neues Registry-Feld `audience` neben `sensitivity`), Share-Seite `/brand/share/:token` (Backend existiert), Print-CSS, keine neue KI. BEFUND `share.post.ts` friert `sensitivity: internal`-Werte ein — von der Nachbar-Sitzung mit MV1 M5 geschlossen (`brandShareableSlotValues`); **G1 FERTIG 2026-09-06** (Davids Go): Registry-Feld `audience` (31 foundation / 37 internal, Ausnahmetabelle + Test), `sessionTravels` als EINE Regel hinter dem Share-Filter, purer Renderer `buildBrandFoundation` (21 Prüfungen), Live-Beweis am Dev-Snapshot; Plan §7. **G2 GEBAUT 2026-09-06** (Route + Seite `/brand/:id/foundation`, `BwFoundationChapter`, generische `BwReadingToc`, Rail-Einstieg „Brand Foundation", Export-Menü, Druck; Live-Beweis am Dev-Server, Druck-Falle im Workspace-Layout gefunden+behoben; Plan §8) — **Davids Blick auf die Seite offen** (Gate G2). **G3 GEBAUT 2026-09-07** (Teilen-Dialog, Share-Seite `/brand/share/:token` mit Renderer + Verzeichnis, Schutz-Köpfe, Ereignisse; Beweis-Skript verify-brand-share 36/36 mit roter Gegenprobe; vier Fallen in Plan §9, zwei davon Core: Verbots-Registry für frame-ancestors, no-cache-Plugin respektiert strengere Header) — **Davids Blick auf Dialog und Empfänger-Seite offen.** **G5 GEBAUT 2026-09-07**: `/beispiel/kailua-coffee` (indexierbar, fester Snapshot `shared/examples/kailuaCoffee.ts`, derselbe Renderer, Startseiten-Teaser zurück; Plan §10) — **Davids Inhalts-Gate offen** (Texte der Beispielmarke). **G4 GEBAUT 2026-09-07** (Davids vier Entscheidungen nach Empfehlung, DECISION-LOG: kuratierter Katalog mit sechs Richtungen, drei je Archetyp, kein Preis an der Schranke, Wahl in der Werkstatt; Plan §11/§12; Live-Beweis Karten → Kapitel 10 → Snapshot). **BF1 ist damit komplett gebaut (G1–G5); offen sind nur Davids Blicke** (Leseansicht, Teilen, Beispiel-Inhalt, Richtungswahl) — danach zieht die Zeile nach COMPLETE. Pakete G1–G5; erst Prototyp im Playground (ein Screen je Interaktionstyp). NACHTRAG 2026-09-05 (Davids Wunsch): Inhaltsverzeichnis als `UPageAside` + `UContentToc` — im Prototyp (beide Ansichten, Baustein `FdToc`) UND auf der ECHTEN Dokument-Seite (`BwDocumentToc`, brand-Layer `ui.content: true`); Falle: Nuxt UI registriert Content-Komponenten nur mit diesem Schalter, und ein laufender Dev-Server übernimmt eine Layer-nuxt.config-Änderung nicht (Neustart). | Mittel | L | Ja: Prototyp-Abnahme; später Schranken-Text + Preisanker, Richtungen-Katalog (G4) | [BRAND-FOUNDATION-LESEANSICHT.md](plans/BRAND-FOUNDATION-LESEANSICHT.md) |
