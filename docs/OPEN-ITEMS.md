...
| 2 · S4 | **Zweites Naht-Geheimnis rotieren (`events-sweep`).** Bei der A0-Rotation blieb es liegen; es steht seit der Einrichtung unverändert in `platform.pukalani.app/.env`. Es ist LEICHTER als gedacht: der Cron liest den Schlüssel bei jedem Lauf frisch aus derselben `.env`, ein zeitgleiches Umstellen ist also NICHT nötig (frühere Einschätzung korrigiert). Fensterlos: neuen Wert über Integrationen in die Ablage von `account` (Empfänger nimmt ab da alt UND neu), dann `NUXT_EVENTS_SWEEP_KEY` in der `.env` setzen, dann bei Gelegenheit neu laden. | Niedrig | XS | Nein — Claude kann es | [sharedSeamSecret.ts](../packages/core/server/utils/sharedSeamSecret.ts) |
