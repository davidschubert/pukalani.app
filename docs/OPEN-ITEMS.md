...
| U15 | 2 — Tage | **Die zugesagten Dashboard-Seiten** — IN ARBEIT: **Navigation + Sucheintrag + Weiterleitungen + Zwei-Faktor sind LIVE** (→ COMPLETE). Zuschnitt-Runde ENTSCHIEDEN 2026-08-13: es fehlt nur noch **Zeitzone** (Konto-Einstellung) — danach ist U15 zu. Taxes + Bulk-Logs bewusst abgelehnt (s. Zurückgestellt), Bio/Profil, Tokens, SSO, E-Mail-Settings geparkt mit Bedingung. | Mittel | M (Rest) | Nein (entschieden) | [DASHBOARD-IA.md](plans/DASHBOARD-IA.md) · DECISION-LOG 2026-08-13 |
| U16 | 3 — Stunden | **Zwei-Faktor und „Anmelden mit Google" greifen nicht ineinander.** Appwrite nimmt OAuth-Sessions bewusst von MFA aus (die Session zählt `email`+`oauth2` als zwei Faktoren, Kommentar im Quelltext: „bypass MFA checks"). Wer den Zweitfaktor einschaltet und sich dann mit Google anmeldet, kommt ohne Code hinein. Drei Wege: (a) so lassen und es auf der Sicherheits-Karte ehrlich hinschreiben, (b) Google-Login sperren, sobald Zwei-Faktor an ist, (c) eigene Challenge nach dem OAuth-Rücksprung erzwingen (baut gegen Appwrite an). Empfehlung: (a) — Google prüft selbst zwei Faktoren. | Mittel | S | **Ja: Weg wählen** | U15 Teil 4 in [OPEN-ITEMS-COMPLETE.md](OPEN-ITEMS-COMPLETE.md) |
