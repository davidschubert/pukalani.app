...
| F57 | 3 — Tage | **Die sozialen Mechaniken aus dem Discussions-Konzept Teil 4.** **Alle vier Mechaniken GEBAUT** (2026-08-13/14): Emoji-**Reaktionen**, **Einladungen durch Mitglieder** (`promoter`), **Tages-Limit für Likes** (50/Tag, UTC-Tag, keine Erstattung) und die **Themen-Verlinkung mit Rückverweis** (`#<id>-<deko>` als Text, Rückverweis-Index posts-020, `first-link`). Offen nur noch: Campaigner/Champion. | Mittel | M | Ja: Reaktionen auf Antworten? + Campaigner/Champion (brauchen die Vertrauensstufe der Eingeladenen — eigener Verleihungs-Pfad) | [DISCUSSIONS-KONZEPT.md](plans/DISCUSSIONS-KONZEPT.md) Teil 4/5 |
