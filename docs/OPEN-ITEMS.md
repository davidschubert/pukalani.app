...
| 9 · MV1 | **Marktvergleich — Prototyp M0/M0b von David FREIGEGEBEN (2026-09-05); Paket M1 „Layer + Vertrag" GEBAUT: Layer `market` montiert in `apps/branding`, expliziter Vertrag zu `brand` (Abruf, Besitz, Slot-Registry, Profil-Kaskade), drei Tabellen market-001…003 (Schema-Anhang B im Plan — David liest gegen).** Als Nächstes **M2** (Abruf + Extraktion: robots/TDM, Sitemap/llms.txt/JSON-LD als GETEILTE Erweiterung mit 8 · BC1 abstimmen, PII-Filter, Beleg-Riegel), danach M3–M6. **Prod-Migration `market` auf `branding` steht aus** (braucht Davids Ja). | Mittel | L | Ja: Schema-Anhang B + Prod-Migration freigeben | [BRAND-MARKTVERGLEICH.md](plans/BRAND-MARKTVERGLEICH.md) |
