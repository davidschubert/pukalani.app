...
| 13 · NAV1 | **Navigation anpassen — vier Entscheidungen GEFALLEN 2026-09-08 (DECISION-LOG), Paket 1 LIVE.** Befund: der Navigations-Editor war auf branding.supply leer (brand registriert nichts in `chrome.nav`, nur Rechtsseiten als CMS) und wirkungslos (einziger Leser = blueprint-Layout; `BwSiteNav` feste Liste; Silo-PATCH 404). **Paket 1 (Wording „Website-Einstellungen" im Silo, `instanceTabs` an in branding + portfolio) auf main (4a4e8d7f).** **Paket 2 in Umsetzung:** Unterpunkte (eine Ebene, Gruppen `group-n`, Silo-Ablage `_instance`, EIN Composable `useCommunityNav`, Renderer blueprint + BwSiteNav) — Prototyp-Screenshots = Davids Freigabe vor dem Merge. **Paket 3 offen:** Reihenfolge der Dashboard-Navigation je Person (Prefs), gilt für alle Sites. Nebenbefund: „Sucheintrag" und „Weiterleitungen" haben im Silo dieselbe 404 beim Speichern (dieselbe rowId-Regel nachziehen). | Hoch | M je Paket | Ja: Freigabe Paket 2 am Prototyp | [NAVIGATION-UNTERPUNKTE.md](plans/NAVIGATION-UNTERPUNKTE.md) |
