...
| 14 · BK1 | **Brand Book & Kit (Produkt 03) — Strategie FREIGEGEBEN 2026-09-09** (acht Entscheidungen, DECISION-LOG): EIN Produkt als dritte Schicht in `brand`, Kit = `brand.md` + `brand.json` + `tokens.json` (DTCG) + `tokens.css` + Zeichen-SVGs + `LICENSES.md`, Preis = BS1-Einmalpreis „Ableitung" 149 € netto (erst mit Z1 sichtbar), zwei KI-Kapitel (Otto/Nika), Kompass + 90-Tage-Plan nach Produkt 04. **Nächster Schritt: Konzeption (§2) auf Davids Startwort.** Vermerk: „Brand Extract" (Ist-Zustand einer Website lesen) wird eigener Baustein NACH BK1. | Mittel | L | Ja: Startwort Konzeption, Inhalts-Gates Otto/Nika | [BRAND-BOOK-KIT.md](plans/BRAND-BOOK-KIT.md) |
