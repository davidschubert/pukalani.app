...
| 9 · MV1 | **Marktvergleich — M1 bis M5 GEBAUT.** M5: Rohtext-Sweep (24-h-Frist, Takt 30 min + Betreiber-Knopf), Erklärseite `/market-bot` (bewusst OHNE Produkt-Gate), Vertraulichkeits-Nachweis (Befund: die vier `internal`-Sessions reisten im Teilen-Link mit — additiv gefixt in `brand/shared/brandSharing.ts`), freiwillige Bewertung in `brand_events`, Runbook + interne Doku; Vertrag Anhang F, `verify-market-retention.mjs`. Offen: **Prod-Migrationen `market-001…004` + `brand-018/019` auf `branding`** und danach der Gate-Flip `pukalani.market.enabled` (eigener Commit, Runbook), **Seiten-/Klick-Abnahme durch David**, **M6 Bibliothek**. | Mittel | M | Ja: Prod-Migrationen freigeben + Seiten-Abnahme | [BRAND-MARKTVERGLEICH.md](plans/BRAND-MARKTVERGLEICH.md) · [Runbook](runbooks/MARKTVERGLEICH-EINFUEHRUNG.md) |
