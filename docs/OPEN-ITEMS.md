...
| 5 · AH-4c | **Appwrite-Projekt `control` → `admin` umziehen** (Davids Entscheidung 2026-08-18, rundet AH-4b ab). Neues Projekt + Daten/Nutzer/neue Keys + Env-Schnitt; die Risiko-Stelle ist die platform-Naht (Mandanten-Auflösung). Inventar klein (41 Tabellen · 178 Rows · 5 Buckets). Vorarbeit (Transfer-Werkzeug) läuft ohne Fenster; der Schnitt braucht ein Wartungsfenster + Davids Konsolen-Klicks. | Mittel | M | Ja: Fenster + Projekt/Keys anlegen | [ADMIN-PROJEKT-CUTOVER.md](runbooks/ADMIN-PROJEKT-CUTOVER.md) |
