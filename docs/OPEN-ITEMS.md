...
| AW2 | 6 — Tage | **Appwrite 2.0 — Server und SDKs.** Server (dev + prod) steht auf 1.9.6; `node-appwrite 29` und `appwrite 27` verlangen Response-Format 2.0.0, `node-appwrite 27/28` entfernen `Health` (Admin-Health-Pillen) und `account.createJWT` (Realtime-Token, Service-Naht). Reihenfolge: Health auf REST + `users.createJWT` ⇒ node-appwrite 28 (läuft noch gegen 1.9.6) ⇒ Server 2.0 dev, dann prod (gemountete Patches registers.php/Traefik mit) ⇒ node-appwrite 29 + appwrite 27. Die Framework-Runde (Nuxt 4.5.2, Vue 3.5.42, UI 4.11.1, Pinia 4.0.3, Zod 4.5.4) ist am 2026-09-07 durch. | Mittel | M | Ja: Go für das Server-Upgrade dev+prod | Davids Go |
