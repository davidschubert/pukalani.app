...
| 5 · AH-4c | **Appwrite-Projekt `control` → `admin`: Umzug GEFAHREN (2026-08-18, ~15 min Fenster, alle Proben grün).** Offen ist nur noch das Aufräumen nach der Beobachtung (~01.09.): Alt-Keys widerrufen, dedizierten rows.read-Key für die platform-Naht tauschen (läuft interim auf dem Runtime-Key), `app_secrets` rotieren. Dazu Vorbestand: `NUXT_TICKETS_AI_KEY` fehlt auf der admin-Site (Davids OpenRouter-Key). | Mittel | S (Rest) | Ja: Keys/Beobachtung | [ADMIN-PROJEKT-CUTOVER.md](runbooks/ADMIN-PROJEKT-CUTOVER.md) Schritt 6 |
