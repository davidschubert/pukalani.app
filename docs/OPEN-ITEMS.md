...
| --- | --- | --- | --- | --- | --- |
| 1 · A0 | **Zugänge aus den Env-Dateien holen — Rest.** Der Reiter „Integrationen" steht (Instanz → Integrationen): KI, Statistik und Konsolen-KI lassen sich dort eintragen, verschlüsselt in `instance_secrets`. Offen sind die drei bewusst ausgelassenen: **SMTP** (der Digest-Sweep läuft ohne Request, der Speicher verlangt heute ein `H3Event`) und die **geteilten Naht-Geheimnisse** (`onboardingServiceSecret`, `eventsSweepKey` — brauchen eine Stufe, in der beide Seiten alt UND neu annehmen). Nicht dringend: alles läuft über die Env weiter. | Niedrig | M | Nein | [instanceSecrets.ts](../packages/core/server/utils/instanceSecrets.ts) |
