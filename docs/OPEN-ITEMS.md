...
| --- | --- | --- | --- | --- | --- |
| 0 · S2 | **system-Migrationen auf `account` sind BLOCKIERT.** `system-031` bricht beim Wiederholungslauf mit 400 ab: EINE `community_handles`-Zeile trägt `communityId = "t-mspkq1xd44lixo8oh2"` — eine **tenantId** statt der `communities.$id`, `Role.label("t-…")` ist ungültig (Bindestrich). Kur: Datenzeile korrigieren UND 031 gegen label-untaugliche Ids härten (überspringen + melden statt werfen). | Hoch | S | Nein | Befund 2026-08-19 (Favicon-Sitzung); system-037 wurde als Einzeldatei vorbeigefahren |
| 1 · S1 | **Alter Sicherungsordner mit lebenden Geheimnissen.** `/home/ploi/.env-backups/` (26. Juli, Control-Cutover) hält fünf Dateien mit u. a. Stripe-Secret, Entitlements-Privatschlüssel und SMTP-Passwort. Geprüft: jeder Wert ist entweder identisch mit der lebenden `.env` oder ein toter Altwert — Löschen verliert nichts. | Mittel | XS | Ja: ein Ja genügt, dann `shred` | Befund 2026-08-19 in dieser Sitzung |
