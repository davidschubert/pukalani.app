...
| --- | --- | --- | --- | --- | --- |
| 1 · A0 | **KI-Schlüssel auf dem `platform`-Server fehlt.** `apps/platform` erklärt `pukalani.ai.enabled: true` und **verkauft `ai` ab Pro (149 €)** — ohne `NUXT_AI_KEY` ist aber `isAiAvailable()` false, und alle drei Verbraucher sind dunkel: Moderations-Assist für Kommentare, derselbe für Beiträge, Übersetzungs-Vorschlag der Kategorien. Nichts wird rot, die Knöpfe erscheinen einfach nicht (F44-Muster, nur teurer). Gefunden auf freelancer.supply (Plan: pro). `pnpm ops:site-env platform` mahnt es jetzt an. | Hoch | S | Ja: Schlüssel setzen (OpenRouter) | [verify-site-env.mjs](../scripts/ops/verify-site-env.mjs) |
...
| U21 | 2 — Tage | **Tickets, die sich selbst umsetzen.** Ein Knopf am Ticket startet Claude Code auf Davids Mac — eigener Worktree, Beschreibung als Auftrag, Anhänge als Material, Modell und Berechtigungs-Modus aus dem Ticket; Fortschritt live im Board, danach Branch, Commit, Diffstat, Tests und Kosten im Ticket. Das Board steht schon, es fehlt die Ausführungs-Schicht: neuer Layer `runner` (Titel „AI-Runner") plus ein kleiner Dienst auf dem Mac (die Website kann selbst nichts starten). Entscheidungen gefallen (2026-08-17): eigener Layer `runner` · Polling statt Realtime · headless zuerst; CLI-Verhalten in drei Testläufen nachgemessen. **Zwei Riegel sind Pflicht:** die Liste erlaubter Repos liegt auf dem Mac, nicht in der Datenbank, und Gast-Feedback darf nie zur Anweisung werden. | Mittel | M | Nein (Entscheidungen sind gefallen) | [AI-RUNNER.md](plans/AI-RUNNER.md) |
