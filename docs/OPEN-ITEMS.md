...
| 13 · NAV1 | **Navigation anpassen — alle drei Pakete GEBAUT** (Wording · Site-Nav mit Unterpunkten · Dashboard-Nav je Person, 2026-09-08; Einträge + Gelernt in COMPLETE). Paket 3 LIVE seit Build 5bbb3b8f, Klick hinter dem Login auf branding.supply bewiesen (2026-09-09). **Offen bleibt EINE Entscheidung:** die BESCHREIBUNG aus dem Sucheintrag liest nur die Pool-Startseite (apps/platform) — auf Silo-Sites mit eigener Startseite wirkt nur `noindex`. Soll die Community-Beschreibung dort die Seiten-Beschreibung überschreiben? | Mittel | S | Ja: Entscheidung Sucheintrag-Beschreibung | [DASHBOARD-NAV-JE-PERSON.md](archiv/DASHBOARD-NAV-JE-PERSON.md) · [NAVIGATION-UNTERPUNKTE.md](archiv/NAVIGATION-UNTERPUNKTE.md) |
| 14 · BK1 | **Brand Book & Kit (Produkt 03) — Strategie FREIGEGEBEN, Konzeption GESCHRIEBEN + sechs Konzept-Fragen beantwortet (alles 2026-09-09, DECISION-LOG).** EIN Produkt als dritte Schicht in `brand` (Kapitel Nomenklatur/AI-Guidelines/Pressekit + Lieferseite `/brand/:id/kit`), Kit = `brand.md` + `brand.json` + `tokens.json` (DTCG, hell/dunkel als Gruppen) + `tokens.css` + Zeichen-SVGs + `LICENSES.md` + Zip, Freischaltung „Ableitung" als Feld brand-025 (Beta/Betreiber/Z1), Preis 149 € netto erst mit Z1 sichtbar. **Nächster Schritt: Davids Konzept-Freigabe ⇒ Prototyp im Playground (§2.15), dann Pakete K0–K8 (§2.18).** Vermerk: „Brand Extract" wird eigener Baustein NACH BK1. | Mittel | L | Ja: Konzept-Freigabe, Prototyp-Abnahme, Inhalts-Gates Otto/Nika | [BRAND-BOOK-KIT.md](plans/BRAND-BOOK-KIT.md) |
