...
| 9 · MV1 | **Marktvergleich — M1 (Layer + Vertrag) und M2 (Abruf + Extraktion) GEBAUT.** M2: robots/TDM/Sperrliste/PII, geteilter Mehrseiten-Abruf mit Sitemap+llms.txt+JSON-LD im brand-Layer (BC1 darf ihn übernehmen), Beleg-Riegel + Häufigkeit deterministisch, KI-Aussensicht mit Konsens-Filter, Eimer aus M5 vorgezogen; Verträge in Anhang B+C des Plans, `verify-market-fetch.mjs` 33/33. Als Nächstes **M3** (Vergleich + Befunde), danach M4–M6. **Prod-Migration `market` auf `branding` steht weiter aus** (braucht Davids Ja). | Mittel | L | Ja: Schema-Anhang B + Prod-Migration freigeben | [BRAND-MARKTVERGLEICH.md](plans/BRAND-MARKTVERGLEICH.md) |
